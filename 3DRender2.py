...
while True: 
    ...
    toBeRenderd = []  # renderBuffer after all of the clipping is done
...
#            Canvas.blit(VCR_MONO.render(str(toBeRenderd[ii][0][0]), False, 'red'), (0,48 * ii))
    ...
    for ii in range(len(toBeRenderd)):
        for iii in range(len(toBeRenderd) - ii):
            if toBeRenderd[ii][2] <= toBeRenderd[iii + ii][2]:
                sWitch = toBeRenderd[iii + ii]
                toBeRenderd[iii + ii] = toBeRenderd[ii]
                toBeRenderd[ii] = sWitch
    ...
    # Apply perspective to the processed vertices using tanFov and the depth of the vertex and translating that to the 2D plane. Why the 10000 in the else statement? idk
    for ii in toBeRenderd:
        for iii in range(len(ii[0])):
            if ii[0][iii](2, 0) > 0:
                Projev.setIndex((0, 0), Resolution[0] / (tanFov * ii[0][iii](2, 0)))
            else:
                Projev.setIndex((0, 0), 10000)
            Projev.setIndex((1, 1), Resolution[0] / (tanFov * ii[0][iii](2, 0)))
            ii[0][iii] = mat.matrixMult(Projev, ii[0][iii])
...
#            (temp[0]+ii[0][0](0,0), temp[1]+ii[0][0](1,0)), (temp[0]+ii[0][1](0,0), temp[1]+ii[0][1](1,0)), (temp[0]+ii[0][2](0,0), temp[1]+ii[0][2](1,0))
    ...
    Canvas.blit(VCR_MONO.render(str(len(toBeRenderd)), False, 'red'), (0, 48))
