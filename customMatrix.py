class Matrix:
    ...
    def __call__(self, y, x):
        ...
        if x < len(self.Content) and y < len(self.Content[0]):
            ...
            raise Exception(f"getIndex exception: tried to get value of index {str((y, x))} of a Matrix with size {str((len(self.Content[0]), len(self.Content)))} (mat:\n{str(self)})")
    ...
    def getIndex(self, y, x):
        ...
        if x < self.width and y < self.height:
            ...
            raise Exception(f"getIndex exception: tried to get value of index {str((y, x))} of a Matrix with size {str((self.height, self.width))}")
    ...
    def setIndex(self, cord, val):
        ...
        if cord[0] < self.height and cord[1] < self.width:
            ...
            raise Exception(f"setIndex exception: tried to put value into index {str((cord[1], cord[0]))} of a Matrix with size {str((self.height, self.width))}")
    ...
    def getColumn(self, x):
        ...
        if x < self.height:
            ...
            raise Exception(f"getColumn exception: tried to get collumn {str(x)} of a Matrix that has {str(self.height)} collumns")
    ...
    def setColumn(self, x, newColumn):
        ...
        if x < self.height and len(newColumn) == len(self.Content[x]):
            ...
            raise Exception(f"setColumn exception: tried to put a collumn with size {str(len(newColumn))} into collumn number {str(x)} of a Matrix with size {str((self.height, self.width))}")
    ...
    def getRow(self, y):
        ...
        if y < self.height:
            ...
            raise Exception(f"getRows exception: tried to get row {str(y)} of a Matrix that has {str(self.height)} rows")
    ...
    def setRow(self, y, newRow):
        ...
        if y < self.height and len(newRow) == self.width:
            ...
            raise Exception(f"setRows exception: tried to put a row with size {str(len(newRow))} into row number {str(y)} of a Matrix with size {str((self.height, self.width))}")
...
def matrixSums(matrix1, matrix2):
    if matrix1.getSize() == matrix2.getSize():
        ...
        raise Exception(f"matrixSums exception: tried to sums {matrix1.getSize()} matrix and {matrix2.getSize()} matrix")
...
def dotProduct(matrix1, matrix2):
    if matrix1.getSize() == matrix2.getSize():
        ...
        raise Exception("Matrices size differs")
...
def matrixMult(matrix1, matrix2):
    if matrix1.width == matrix2.height:
        ...
        raise Exception("1st Matrix's width is not the same length as 2nd Matrix's height")
...
def vdotProduct(vector1, vector2):
    if len(vector1) == len(vector2):
        ...
        raise Exception("Vectors size differ")
...
def crossProduct(vector1, vector2):
    if vector1.height == 3 and vector2.height == 3 and vector1.width == 1 and vector2.width == 1:
        ...
        raise Exception("Inputed vectors contain something other than 1x3 Matrices (3d Vectors): {}")
